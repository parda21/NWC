/* NWC2-Dekoder v3: wie v2, aber jeder Thread dekodiert ILP unabhaengige Teilstroeme
   verschraenkt (Lane k der Bloecke g*ILP .. g*ILP+ILP-1). Die seriellen Ketten
   ueberlappen sich, die Latenz von Shared-Zugriff und Multiplikation wird versteckt.
   Keine Formataenderung noetig.                                                     */
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cstdint>
...
kern_rans16_ilp(const uint8_t  *__restrict__ daten,
                ...
                uint64_t n_high, uint32_t block, uint32_t lanes, uint32_t n_bloecke)
{
    ...
    uint64_t g        = blockIdx.x * (uint64_t)blockDim.x + threadIdx.x;   /* (Gruppe, Lane) */
    uint64_t gruppen  = ((uint64_t)n_bloecke + ILP - 1) / ILP;
    uint64_t gesamt   = gruppen * lanes;
    uint64_t schritt  = gridDim.x * (uint64_t)blockDim.x;

    for (; g < gesamt; g += schritt) {
        uint32_t gruppe = (uint32_t)(g / lanes);
        uint32_t lane   = (uint32_t)(g % lanes);

        const uint8_t *p[ILP];
        uint8_t       *out[ILP];
        uint32_t       x[ILP], n_lane[ILP];
        uint32_t       n_max = 0;

        #pragma unroll
        for (int j = 0; j < ILP; j++) {
            uint32_t b = gruppe * ILP + j;
            n_lane[j] = 0; x[j] = 0; p[j] = daten; out[j] = res;
            if (b < n_bloecke) {
                uint64_t basis = (uint64_t)b * block;
                uint64_t rest  = n_high - basis;
                uint32_t n_sym = (rest > block) ? block : (uint32_t)rest;
                if (lane < n_sym) {
                    n_lane[j] = (n_sym - lane + lanes - 1) / lanes;
                    const uint8_t *q = daten + basen[b] + subrel[(uint64_t)b * lanes + lane];
                    x[j] = ((uint32_t)q[0] << 24) | ((uint32_t)q[1] << 16) |
                           ((uint32_t)q[2] <<  8) |  (uint32_t)q[3];
                    p[j] = q + 4;
                    out[j] = res + basis + lane;
                    if (n_lane[j] > n_max) n_max = n_lane[j];
                }
            }
        }
        ...
        for (uint32_t i = 0; i < n_max; i++) {
            ...
            for (int j = 0; j < ILP; j++) {
                if (i < n_lane[j]) {
                    uint32_t e = s_tab[x[j] & (TOT - 1)];
                    out[j][i * lanes] = (uint8_t)e;
                    uint32_t xn = (((e >> 8) & 0xFFFu) + 1u) * (x[j] >> SCALE_BITS) + (e >> 20);
                    if (xn < RANS_L16) { xn = (xn << 16) | ((uint32_t)p[j][0] << 8) | p[j][1]; p[j] += 2; }
                    x[j] = xn;
                }
            }
        }
...
int main(int argc, char **argv) {
    if (argc < 3) { printf("nutzung: nwcdec_cuda3 <datei.nwc> <referenz.raw>\n"); return 2; }
    ...
    if (renorm != 16) { printf("v3 braucht renorm=16, Datei hat %u\n", renorm); return 1; }
    ...
    printf("Datei          : %.1f MB, %u Bloecke x %u Lanes, ILP=%d -> %llu Threads\n",
           fg / 1e6, n_bloecke, lanes, ILP,
           (unsigned long long)(((n_bloecke + ILP - 1) / ILP) * (uint64_t)lanes));
    ...
    uint64_t n_threads = (((uint64_t)n_bloecke + ILP - 1) / ILP) * lanes;
    int th = 256;
    ...
    for (int w = 0; w < 10; w++) {
        cudaEventRecord(e[0]);
        kern_rans16_ilp<<<bl_a, th>>>(d_daten, d_basen, d_sub, d_tab, d_res, n_high, block, lanes, n_bloecke);
        cudaEventRecord(e[1]);
    ...
    printf("  Stufe A rANS16 ILP%d : %7.3f ms   %8.1f GB/s Ausgabe\n", ILP, ms_a, mb / ms_a);
