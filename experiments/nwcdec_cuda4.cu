/* NWC2-Dekoder v4 fuer Formatversion 3 (ausgerichtete Teilstroeme, little-endian).
   Nachladen ueber einen Bitpuffer mit ausgerichteten 32- oder 64-Bit-Ladebefehlen
   statt zweier einzelner Bytes -> 2-4x weniger L1-Wavefronts.
   Kompilieren mit -DLOAD=32 oder -DLOAD=64.                                          */
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cstdint>
...
kern_rans16_al(const uint8_t  *__restrict__ daten,
               ...
               uint64_t n_high, uint32_t block, uint32_t lanes, uint32_t n_bloecke)
{
    ...
    for (; tid < gesamt; tid += schritt) {
        ...
        const uint8_t *start = daten + basen[b] + subrel[(uint64_t)b * lanes + lane];  /* 8-ausgerichtet */
        uint32_t x = *(const uint32_t*)start;                                         /* Zustand, LE */
#if LOAD == 64
        uint64_t w = *(const uint32_t*)(start + 4);   int nb = 32;                    /* erstes Wortpaar */
        const uint2 *p = (const uint2*)(start + 8);                                    /* ab hier 8-ausgerichtet */
#else
        uint32_t w = 0; int nb = 0;
        const uint32_t *p = (const uint32_t*)(start + 4);
#endif
        uint8_t *out = res + basis + lane;
        #pragma unroll 4
        for (uint32_t i = 0; i < n_lane; i++) {
            uint32_t e = s_tab[x & (TOT - 1)];
#ifdef NOSTORE                                   /* Diagnose: Dekodieren ohne Schreiben */
            if (e == 0xFFFFFFFFu)
#endif
...
int main(int argc, char **argv) {
    if (argc < 3) { printf("nutzung: nwcdec_cuda4 <datei.nwc> <referenz.raw>\n"); return 2; }
    ...
    if (version != 3 || skala != SCALE_BITS || renorm != 16 || stride != 0) {
        printf("v4 braucht Formatversion 3 (nwcenc), renorm=16, stride=0\n"); return 1;
    }
    ...
    printf("Datei          : %.1f MB, %u Bloecke x %u Lanes = %llu Threads, LOAD=%d\n",
           fg / 1e6, n_bloecke, lanes, (unsigned long long)n_bloecke * lanes, LOAD);
    ...
    for (int w = 0; w < 10; w++) {
        cudaEventRecord(e[0]);
        kern_rans16_al<<<bl_a, th>>>(d_daten, d_basen, d_sub, d_tab, d_res, n_high, block, lanes, n_bloecke);
        cudaEventRecord(e[1]);
    ...
    printf("  Stufe A rANS16 LOAD%d : %7.3f ms   %8.1f GB/s Ausgabe\n", LOAD, ms_a, mb / ms_a);
