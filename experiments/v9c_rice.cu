/* v9c_rice -- Praefixcode-Dekoder, Stufe 2b: Fenster per Funnel-Shift statt IMAD (A16: IMAD/PRMT/SHF halbe Rate,
   IMAD.HI Viertelrate, nur FFMA voll; ALU- und FMA-Pipe muessen ausbalanciert sein, siehe experimente/pipe_bench.cu).
   LUT-Eintrag: 2 Codes:  c | by0<<8 | by1<<16         (c = verbrauchte Bits in Byte 0; shf.l.wrap maskiert selbst auf 5 Bit)
                <2 Codes: FLAG | c1 | by0<<8 | n<<16    (FLAG = Bit 30; Byte 3 hat MSB 0 -> PRMT-Nullbytes per Vorzeichen)
   Fenster hi:lo (MSB zuerst), Fuellstand als Rotation: M = 2^(64-nb) mod 2^32, Schritt M = rotl(M, c); Ueberlauf
   (M_neu < M_alt) <=> nb' <= 32 <=> Nachladen: (hi:lo) += w * M_neu (praediziertes IMAD.WIDE), naechstes Wort laden.
   Blocklayout 8 Zeilen x 512 Spalten wie v9b, Zeilenschleife UNR-fach ausgerollt (Akkumulator-Rotation).
   nvcc -O3 -arch=sm_86 -o build/v9c_rice experimente/v9c_rice.cu [-DUNR=2 -DREFILL=0..2 -DPEEKV=0..2 -DANY2 -DPF -DNOSLOW -DNOREFILL -DNOLDS]
   ./build/v9c_rice data/W.raw M K          (K % 512 == 0, M % 8 == 0)                                        */
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cstdint>
...
#endif
#ifndef REFILL
#define REFILL 0           /* 0: ISETP+SEL + 64-Bit-Add, 1: LOP3-Maske + OR, 2: ISETP+SEL + OR */
#endif
#ifndef PEEKV
#define PEEKV 2            /* 0: SHF+LOP3, 1: IMAD.HI (beide Pipes), 2: SHF mit opakem Shift + LDS.X4 */
#endif
...
static void lut_bauen(const uint8_t *exp_von_rang, uint32_t *lut) {
    for (uint32_t pat = 0; pat < LUTN; pat++) {
        ...
        else        lut[pat] = FLAG | (n ? c1 : 0u) | (by[0] << 8) | (n << 16);
    }
}

/* ---------------- Device ---------------- */
struct Fenster {
    uint64_t hl, p; uint32_t M, w, off; const uint8_t *basis;
    __device__ __forceinline__ uint32_t hi() const { return (uint32_t)(hl >> 32); }
    __device__ __forceinline__ void init(const uint32_t *start, const uint8_t *daten) {
        hl = ((uint64_t)__ldg(start) << 32) | __ldg(start + 1); w = __ldg(start + 2); M = 1;
        p = (uint64_t)(start + 3); basis = daten; off = (uint32_t)((const uint8_t*)(start + 3) - daten);
    }
    /* Fenster um c Bits schieben (c = untere 5 Bit von e) und nachladen: 3 SHF, 1 ISETP, praediziert IMAD.WIDE/LDG/IADD */
    __device__ __forceinline__ void schiebe(uint32_t e) {
        uint32_t M_alt = M, nh, nl, lo = (uint32_t)hl, h = hi();
        asm("shf.l.wrap.b32 %0, %1, %2, %3;" : "=r"(nh) : "r"(lo), "r"(h), "r"(e));
        asm("shf.l.wrap.b32 %0, %1, %2, %3;" : "=r"(nl) : "r"(0u), "r"(lo), "r"(e));
        asm("shf.l.wrap.b32 %0, %1, %1, %2;" : "=r"(M) : "r"(M_alt), "r"(e));
#ifdef NOREFILL
        hl = ((uint64_t)nh << 32) | nl;   /* Diagnose: kein Nachladen (Muell) */
        if (M < M_alt) { hl ^= M; }
#else
#if REFILL == 0        /* ISETP+SEL, 64-Bit-Add (ptxas: IADD3 + IMAD.X) */
        uint32_t mh = (M < M_alt) ? M : 0u;
        hl = ((uint64_t)nh << 32) | nl;
        hl = (uint64_t)w * mh + hl;
        if (M < M_alt) { asm("ld.global.nc.u32 %0, [%1];" : "=r"(w) : "l"(p)); p += 4; }
#elif REFILL == 1      /* LOP3-Maske, OR-Merge */
        uint32_t mh = M & (M_alt - 1u);
        uint64_t t = (uint64_t)w * mh;
        hl = ((uint64_t)(nh | (uint32_t)(t >> 32)) << 32) | (nl | (uint32_t)t);
        if (mh) { asm("ld.global.nc.u32 %0, [%1];" : "=r"(w) : "l"(p)); p += 4; }
#elif REFILL == 2      /* ISETP+SEL, OR-Merge */
        uint32_t mh = (M < M_alt) ? M : 0u;
        uint64_t t = (uint64_t)w * mh;
        hl = ((uint64_t)(nh | (uint32_t)(t >> 32)) << 32) | (nl | (uint32_t)t);
        if (M < M_alt) { asm("ld.global.nc.u32 %0, [%1];" : "=r"(w) : "l"(p)); p += 4; }
#elif REFILL == 3      /* ISETP+SEL, mad.wide per asm (kein 64x32-Muell) */
        uint32_t mh = (M < M_alt) ? M : 0u;
        hl = ((uint64_t)nh << 32) | nl;
        asm("mad.wide.u32 %0, %1, %2, %0;" : "+l"(hl) : "r"(w), "r"(mh));
        if (M < M_alt) { asm("ld.global.nc.u32 %0, [%1];" : "=r"(w) : "l"(p)); p += 4; }
#elif REFILL == 6      /* OR-Merge, 32-Bit-Offset zur uniformen Basis (kein Zeigerpaar) */
        uint32_t mh = (M < M_alt) ? M : 0u;
        uint64_t t = (uint64_t)w * mh;
        hl = ((uint64_t)(nh | (uint32_t)(t >> 32)) << 32) | (nl | (uint32_t)t);
        if (M < M_alt) { w = __ldg((const uint32_t*)(basis + off)); off += 4; }
#elif REFILL == 7      /* wie 0, aber 32-Bit-Offset */
        uint32_t mh = (M < M_alt) ? M : 0u;
        hl = ((uint64_t)nh << 32) | nl;
        hl = (uint64_t)w * mh + hl;
        if (M < M_alt) { w = __ldg((const uint32_t*)(basis + off)); off += 4; }
#else                  /* alles in einem asm-Block: setp, selp, mad.wide, praedizierte LDG und Zeiger */
        hl = ((uint64_t)nh << 32) | nl;
        asm("{ .reg .pred q; .reg .u32 mh; setp.lt.u32 q, %3, %4; selp.u32 mh, %3, 0, q; mad.wide.u32 %0, %1, mh, %0; "
            "@q ld.global.nc.u32 %1, [%2]; @q add.u64 %2, %2, 4; }" : "+l"(hl), "+r"(w), "+l"(p) : "r"(M), "r"(M_alt));
#endif
#endif
    }
#if PEEKV == 1
    __device__ __forceinline__ uint32_t peek4(uint32_t) const { return __umulhi(hi(), 1u << PEEK) * 4u; }
#elif PEEKV == 2
    __device__ __forceinline__ uint32_t peek4(uint32_t sh) const { uint32_t i; asm("shr.u32 %0, %1, %2;" : "=r"(i) : "r"(hi()), "r"(sh)); return i * 4u; }
#else
    __device__ __forceinline__ uint32_t peek4(uint32_t) const { return (hi() >> (32 - PEEK)) << 2; }
#endif
    __device__ __forceinline__ uint32_t code_langsam(const uint8_t *evr) {
        uint32_t h = hi(), u = __clz(h);
        if (u < MAXR) { uint32_t s = (h >> (30 - u)) & 1; uint32_t b = (s << 7) | evr[u]; schiebe(u + 2); return b; }
        schiebe(16); uint32_t b = hi() >> 24; schiebe(8); return b;
    }
};
...
kern(const uint8_t *__restrict__ daten, const uint32_t *__restrict__ basen, const uint8_t *__restrict__ hdr,
     ...
     const uint8_t *__restrict__ exp_von_rang_g, uint32_t sh)
{
    ...
    for (uint32_t b = blockIdx.x * (TBP / 32) + (threadIdx.x >> 5); b < n_items; b += gridDim.x * (TBP / 32)) {
        ...
        Fenster F; F.init((const uint32_t*)(daten + basen[b]) + (xx - l4), daten);
#ifdef PF
        { uint32_t worte = __shfl_sync(0xffffffffu, xx, 31);           /* Blockdaten zeilenweise (128 B) in L1 vorholen */
          if (lane * 32 < worte) asm volatile("prefetch.global.L1 [%0];" :: "l"(daten + basen[b] + lane * 128)); }
#endif
        ...
        for (int r = 0; r < (int)ZEILEN; r++) {
            ...
            for (int q = 0; q < 8; q++) {
                uint32_t e;
#ifdef NOLDS
                e = ((F.peek4(sh) * 2654435761u) & 0xFFFF00u) | 6u;
#else
                asm("ld.shared.u32 %0, [%1];" : "=r"(e) : "r"(sm_lut + F.peek4(sh)));
#endif
#ifdef NOSLOW
                F.schiebe(e);
#else
#ifdef ANY2
                if (__any_sync(0xffffffffu, e >= FLAG)) {
                    if (e >= FLAG) {
                        uint32_t n = (e >> 16) & 1, b0 = (e >> 8) & 0xFFu, b1;
                        if (n) F.schiebe(e);
                        if (!n) b0 = F.code_langsam(evr);
                        b1 = F.code_langsam(evr);
                        e = (b0 << 8) | (b1 << 16);                   /* c = 0: das folgende schiebe ist ein No-op */
                    }
                }
                F.schiebe(e);
#else
                if (e < FLAG) F.schiebe(e);
                else {
                    uint32_t n = (e >> 16) & 1, b0 = (e >> 8) & 0xFFu, b1;
                    if (n) F.schiebe(e);
                    if (!n) b0 = F.code_langsam(evr);
                ...
                if (MODE == 0) {
                    sr = fmaf(gewicht(cw, e, (5 << 12) | ((jj & 3) << 8) | 0xFF), xw[jj], sr);
                    sr = fmaf(gewicht(cw, e, (6 << 12) | (((jj + 1) & 3) << 8) | 0xFF), xw[jj + 1], sr);
                } else w16[q] = (e >> 8) & 0xFFFFu;
            }
            if (MODE == 1) {
...
int main(int argc, char **argv) {
    if (argc < 4) { printf("nutzung: v9c_rice W.raw M K\n"); return 1; }
    ...
    kern<1><<<grid, TBP, sm0>>>(d_daten, d_basen, d_hdr, d_lut, d_low, d_x, d_teil, d_hi, K, n_bl, d_evr, 32 - PEEK);
    ...
    for (int i = 0; i < 5; i++) kern<0><<<grid, TBP, sm0>>>(d_daten, d_basen, d_hdr, d_lut, d_low, d_x, d_teil, d_hi, K, n_bl, d_evr, 32 - PEEK);
    CK(cudaDeviceSynchronize());
    std::vector<float> zeiten;
    for (int i = 0; i < 30; i++) {
        cudaEventRecord(a);
        kern<0><<<grid, TBP, sm0>>>(d_daten, d_basen, d_hdr, d_lut, d_low, d_x, d_teil, d_hi, K, n_bl, d_evr, 32 - PEEK);
